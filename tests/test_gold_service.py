...
def test_build_gold_for_symbol_writes_hashed_parquet_and_manifest(tmp_path: Path) -> None:
    ...
    assert re.fullmatch(rf"{symbol}_[a-f0-9]{{12}}_[A-Za-z0-9]+\.parquet", parquet_name) is not None
    ...
    assert payload["source_silver_datasets"]["spot_1m"]["source_symbols"] == ["BTC_USDC"]
    assert payload["source_silver_datasets"]["perp_1m"]["source_symbols"] == ["BTC-PERPETUAL"]
