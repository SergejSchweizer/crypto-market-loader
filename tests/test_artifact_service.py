...
def _build_funding(count: int) -> list[FundingPoint]:
    ...
    for idx in range(count):
        open_time = start + timedelta(hours=8 * idx)
        output.append(
            FundingPoint(
                exchange="deribit",
                symbol="BTCUSDT",
                interval="8h",
                open_time=open_time,
                close_time=open_time + timedelta(hours=8) - timedelta(milliseconds=1),
...
def test_spot_full_history_plot_is_capped_to_1000_points(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    """Spot plot should span full period and cap to ``MAX_FULL_PLOT_POINTS`` values."""
...
def test_oi_full_history_plot_is_capped_to_1000_points(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    """OI plot should span full period and cap to ``MAX_FULL_PLOT_POINTS`` values."""
...
def test_funding_full_history_plot_is_capped_to_1000_points(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    """Funding plot should span full period and cap to ``MAX_FULL_PLOT_POINTS`` values."""
